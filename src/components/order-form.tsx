...
import { products, packIngredients, packCard, type PriceRow } from "@/lib/catalog";
...
const flavorOptions: FlavorOption[] = [
  ...
  { label: packCard.name, slug: packCard.slug, prices: null, isCustomPack: true },
];
...
export default function OrderForm() {
  ...
  function toggleIngredient(ingredient: string) {
    setPackError(false);
    setChosenIngredients((current) => current.includes(ingredient)
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (isCustomPack && chosenIngredients.length === 0) {
      setPackError(true);
      return;
    }
    ...
    const msg =
      ...
      `🥣 Saveur : ${v("flavor")}\n` +
      (isCustomPack
        ? `✨ Ingrédients choisis : ${chosenIngredients.join(", ")}\n⚖️ Poids : ${v("variant")}\n`
    ...
    <form onSubmit={onSubmit} noValidate className="bg-cream text-ink rounded-[34px] px-[1.9rem] py-8 shadow-[0_26px_64px_rgba(33,30,24,.18)]">
      ...
      {isCustomPack && (
        <fieldset className="mb-5 rounded-2xl border border-primary/15 bg-primary/[.045] p-4">
          <legend className="px-1 font-semibold text-[.9rem] text-ink">Choisis tes ingrédients <span className="text-secondary">*</span></legend>
          <p className="mt-1 text-[.8rem] leading-snug text-muted">Coche ce que tu veux dans ton mélange. Tu peux en choisir plusieurs.</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {packIngredients.map((ingredient) => {
              const isChecked = chosenIngredients.includes(ingredient);
              return (
                <label key={ingredient} className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-2.5 text-[.86rem] font-semibold transition-colors ${isChecked ? "border-primary bg-white text-primary shadow-sm" : "border-ink/10 bg-white/65 text-ink hover:border-primary/40"}`}>
                  <input
                    type="checkbox"
                    name="pack-ingredients"
                    value={ingredient}
                    checked={isChecked}
          ...
          {packError && <p className="mt-3 text-[.8rem] font-semibold text-secondary" role="alert">Choisis au moins un ingrédient pour ton mélange.</p>}
        </fieldset>
      )}

      <div className="mb-2">
        <label htmlFor="of-variant" className="block font-semibold text-[.9rem] mb-1.5">{isCustomPack ? "Poids du mélange" : "Poids & prix"} *</label>
        <select id="of-variant" name="variant" required defaultValue="" key={flavorLabel} className={field}>
          <option value="" disabled>
            {isCustomPack ? "Choisir le poids…" : selected?.prices ? "Choisir le poids…" : "Choisis d'abord une saveur"}
          </option>
          {variantRows.map(([g, p]) => (
        ...
        {isCustomPack ? (
          <p className="text-muted text-[.78rem] mt-1.5">Le total dépend de tes ingrédients : on te le confirme rapidement sur WhatsApp.</p>
        ) : selected?.prices && (
